...
class Spider(Spider):
    ...
    def getName(self): return "KTV歌词音乐"
    ...
    def detailContent(self, ids):
        url = self._abs(ids[0])
        doc = self.getpq(url)
        vod = {"vod_id": url, "vod_name": self._clean(doc("h1").text() or doc("title").text()), "vod_pic": self._abs(doc(".djpg img, .pic img, .djpic img").attr("src")), "vod_play_from": "KTV歌词音乐", "vod_content": ""}
